...
Results are written to `analyzer/misc/mlir-contraction/results/`:
...
MLIR and SALT JSON there, and points `scripts/graph.py` at that staged tree.
...
```bash
MPLCONFIGDIR=/tmp/autolala-matplotlib python3 scripts/graph.py
```

All paths accepted by `graph.py` can be overridden; run
`python3 scripts/graph.py --help` for details.
...
python3 scripts/timing_selected.py \
  --data analyzer/misc/mlir-contraction/timing-selected.json \
  --out analyzer/misc/mlir-contraction/timing-selected.svg
```
...
python3 scripts/measure_timing_selected.py \
  --simulation-times analyzer/misc/mlir-contraction/results/timing-simulation-wall.tsv \
  --simulation-db analyzer/misc/mlir-contraction/results/data-fully-associative.db \
  --out analyzer/misc/mlir-contraction/results/timing-selected.json
```
...
2. A C padding utility exists under `analyzer/misc/einsum/constant_global/`, but
