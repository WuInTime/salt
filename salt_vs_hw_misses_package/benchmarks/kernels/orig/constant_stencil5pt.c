...
static void constant_stencil5pt_init(void)
{
...
static void constant_stencil5pt_execute(void)
{
...
static void constant_stencil5pt_cleanup(void)
{
    volatile double sink = B[DIM_N - 2][DIM_M - 2];
    fprintf(stderr, "Result: %f (ignore)\n", sink);
}

static void constant_stencil5pt_verify(void)
{
    // Optional: Add verification logic here
}

kernel_t constant_stencil5pt_kernel = {
    .name = "constant_stencil5pt",
    .description = "Baseline 5-point stencil (512x512)",
    .init = constant_stencil5pt_init,
    .execute = constant_stencil5pt_execute,
    .cleanup = constant_stencil5pt_cleanup,
    .verify = constant_stencil5pt_verify
};
